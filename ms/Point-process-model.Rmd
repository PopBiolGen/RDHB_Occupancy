...
gives the density of individuals at each survey location.  We can then treat the number of individuals present at $s_{i,t}$ as a Poisson distributed variable.  In the common event that we do not have count data, we can express the poisson rate as a probability that at least one individual is present at $s_{i,t}$: $\gamma_{i,t} = 1-e^{-u_{i,t}}$.  We can then treat the presence/absence of at least one individual as a latent variable:

$$
p_{i,t} \sim \text{Bernoulli}(\gamma_{i,t})
$$

We can then proceed to the likelihood for each of our observations by defining a detection function yielding the probability of detection given presence, $\psi_{i,t}$.  This leads us, finally to the full likelihood for each of our observations:

$$
o_{i,t} \sim \text{Bernoulli}(p_{i,t}\psi_{i,t})
$$
