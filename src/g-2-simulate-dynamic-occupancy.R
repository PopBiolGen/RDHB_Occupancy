...
rm(col.int, col.space, ext.int, ext.b, det.int, det.b1, det.b2, k)
...
o.state <- dyn.occ(init.site = init.occ, nsteps = TT, dm = dist.mat, ext.vars = ext.vars, col.vars = col.vars, pars = p.list)
...
for (jj in 1:JJ){
  for (tt in 1:TT){
    occ <- rbinom(1, 1, o.state[jj, tt]) #occupied, or not?
    ...
    det.vars <- det.vars[1:k.realised, ,drop = FALSE]
...
rm(p.obs, k.realised, jj, tt, init.occ, prob.det, pred.det)
